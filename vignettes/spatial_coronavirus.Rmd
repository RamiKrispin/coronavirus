...
To see the current distribution of cases around the world, we'll start by updating the coronavirus dataset and then making it spatial.
...
#as the package might not have the most
#up to date data
...
## Single Country
What if we had wanted to just do a single country? Let's look at the continental US, for example. Rather than doing a lot of string manipulation on the `Province.State` column, we can use `rnaturalearth` with an `st_join` to get the info we want for grouping calculations.
