...
* **DataFrames** - to store the data as data frame and manipulate it (filter, group by, etc.)
...
We will start by loading the CSV file from the repository. Since we are pulling the CSV file from a web, as opposed to loading locally, we will have to use the `download` function to download file and then read it as CSV file:
``` julia
...
We will use the `describe` function to review the data frame attributes:
