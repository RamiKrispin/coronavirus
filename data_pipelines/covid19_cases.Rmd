...
coronavirus <- coronavirus_temp %>%
  dplyr::mutate(index = 1:nrow(coronavirus_temp)) %>%
...
load("data_pipelines/log.RData")

log_last <- log %>% dplyr::filter(update == TRUE & success == TRUE) %>%
...
if(s && nrow(coronavirus) > log_last$nrows){
  log <- rbind(log, data.frame(time = Sys.time(), 
                    ...
                    branch = branch,
                    stringsAsFactors = FALSE))
  save(log, file = "data_pipelines/log.RData")
